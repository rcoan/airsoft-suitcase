...
def create_power_system():
    ...
    with schemdraw.Drawing(file='diagrams/power_system.svg') as d:
        # Power source
...
def create_arduino_pinout():
    ...
    with schemdraw.Drawing(file='diagrams/arduino_pinout.svg') as d:
...
def create_keypad_matrix():
    ...
    with schemdraw.Drawing(file='diagrams/keypad_matrix.svg') as d:
...
def create_lcd_i2c():
    ...
    with schemdraw.Drawing(file='diagrams/lcd_i2c.svg') as d:
        # Arduino I2C pins
        ...
        d += (R1 := elm.Resistor().up().label('4.7kOhm'))
        d += elm.Line().right(d.unit*0.5)
        d += elm.Line().to(VCC.start)
        
        d += elm.Line().up(d.unit*0.3).at(SCL_PIN.start)
        d += (R2 := elm.Resistor().up().label('4.7kOhm'))
        d += elm.Line().right(d.unit*0.5)
        d += elm.Line().to(VCC.start)

def create_buzzer_led():
    """Generate buzzer and LED circuit diagram"""
    with schemdraw.Drawing(file='diagrams/buzzer_led.svg') as d:
        # Arduino pin 13
        ...
        d += (RESISTOR := elm.Resistor().right().label('220Ohm'))
...
def create_siren_relay():
    ...
    with schemdraw.Drawing(file='diagrams/siren_relay.svg') as d:
        # Arduino control
        d += (ARDUINO := elm.RBox().label('Arduino\nPin 12').left())
        d += elm.Line().right(d.unit*1)
        d += (PIN12 := elm.Dot().label('Pin 12'))
...
def create_system_overview():
    ...
    with schemdraw.Drawing(file='diagrams/system_overview.svg') as d:
        # Main power system
...
def main():
    ...
    # Generate all diagrams (they save themselves now)
    create_power_system()
    create_arduino_pinout()
    create_keypad_matrix()
    create_lcd_i2c()
    create_buzzer_led()
    create_siren_relay()
